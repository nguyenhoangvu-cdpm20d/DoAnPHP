...
<head>
	...
	<!-- My CSS -->
	<link rel="stylesheet" href="{{asset('teamplateAdmin/style.css')}}">
...
<body>
        ...
        <li class="">
            ...
            <li>
                <a href="{{route('loai-do-vat')}}">
                    <i class='bx bxs-book'></i>
                    <span class="text">Quản lý đồ vật</span>
                </a>
            </li>
            <li>
				<a href="{{route('tin-tuc')}}">
					<i class='bx bxs-news'></i>
					<span class="text">Quản lý loại tin tức</span>
				</a>
			</li>
			<li>
				<a href="{{route('quan')}}">
	...
	<section id="content">
		...
		<!-- MAIN -->
		<main>
			<form method="Post" action="{{route('.update-loaibaiviet',['id'=>$Loaibaiviet->id])}}">
				@csrf
                
				<div class="col-sm-12 col-xl-6">
					<div class="bg-light rounded h-100 p-4">
						<h6 class="mb-4">Chỉnh sửa Loại Bài Viết</h6>
						<div class="input-group mb-3">
							<input type="text" class="form-control" placeholder="{{$Loaibaiviet->tenloaibaiviet}}" name="tenloaibaiviet" aria-describedby="basic-addon1">
						</div>
						@if(session('Error'))
						<p style="color:red">{{session('Error')}}</p>
						@endif
						<br>
						<button type="submit" class="btn btn-success order">Chỉnh sửa</button>
					</div>
				</div>
			</form>
		</main>
