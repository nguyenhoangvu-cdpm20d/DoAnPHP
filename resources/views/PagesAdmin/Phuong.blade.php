...
<body>
	...
	<section id="content">
		...
		<main>
			...
			<a href="{{route('.add-phuong')}}"><button type="button" class="btn btn-primary" data-toggle="button" aria-pressed="false" autocomplete="off">
					Thêm Phường
				</button></a>
			<div class="table-data">
				<div class="order">
					...
					<table>
						...
						<tbody>
							@foreach($phuong as $List)
							<tr>
								<td>{{$List->id}}</td>
								<td>{{$List->tenphuong}}</td>

								<td>
								<a href="{{('/update_phuong/'.$List->id)}}" class="btn btn-outline-success">Chỉnh Sửa</a>
                                    <a href="/delete_phuong/{{$List->id}}" class="btn btn-outline-danger">Xoá</a>
								</td>
							</tr>
